...
content_overview  <- read_csv("clean-data/content-overview.csv")
...
```{r}
impressions_deepdive  <- content_posts |>
  mutate(week = floor_date(date, unit = "week", week_start = 1)) |>
  group_by(week) |>
  mutate(total_impressions = sum(impressions, na.rm = TRUE)) |>
...
cor(content_posts$impressions, content_posts$likes, use = "complete.obs")
```

```{r}
content_posts_less7k <- content_posts |>
  filter(impressions < 7000)

content_posts_less7k |>
  ggplot(aes(x = impressions, y = likes)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_few()

cor(content_posts_less7k$impressions, content_posts_less7k$likes, use = "complete.obs")
```

```{r}
paste0("Correlation Impressions / Likes: ", cor(content_posts$impressions, content_posts$likes, use = "complete.obs"))
paste0("Correlation Impressions / Comments: ", cor(content_posts$impressions, content_posts$comments, use = "complete.obs"))
paste0("Correlation Impressions / Reposts: ", cor(content_posts$impressions, content_posts$reposts, use = "complete.obs"))
paste0("Correlation Impressions / Clicks: ", cor(content_posts$impressions, content_posts$clicks, use = "complete.obs"))
paste0("Correlation Impressions / Engagement Rate: ", cor(content_posts$impressions, content_posts$engagement_rate, use = "complete.obs"))
```
...
```{r}
hist(content_posts$click_through_rate_ctr, main = "Distribution of CTR", xlab = "Click Through Rate")
```
...
paste0("Correlation: ", cor(content_posts$impressions, content_posts$click_through_rate_ctr, use = "complete.obs"))
round(quantile(x = content_posts$click_through_rate_ctr, probs = seq(0,1,.1), na.rm = TRUE), digits = 2)
```

```{r}
click_through_no_outliers  <- content_posts |>
...
paste0("Correlation: ", cor(click_through_no_outliers$impressions, click_through_no_outliers$click_through_rate_ctr, use = "complete.obs"))
```
